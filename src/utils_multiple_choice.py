...
class DataProcessor(object):
    ...
    def __init__(self, prob_labels=False):
...
class HellaSwagProcessor(DataProcessor):
    ...
    def __init__(self, prob_labels = False):
        super(HellaSwagProcessor, self).__init__(prob_labels)
        if prob_labels:
            print("Reading Probabilistic labels")
    ...
    def get_train_examples(self, data_dir, data_file):
        ...
        if self.prob_labels:
            return self._create_examples(self._read_json(os.path.join(data_dir, data_file)),
                                         self._read_json(os.path.join(data_dir, data_file).replace('.jsonl', '-logits.txt')))
        else:
            return self._create_examples(self._read_json(os.path.join(data_dir, data_file)))
    ...
    def _create_examples(self, lines: List[List[str]], labels = None):
        ...
        examples = []
        for i, line in tqdm.tqdm(enumerate(lines), desc="read hellaswag data"):
            data_raw = json.loads(line.strip("\n"))

            if labels is not None:
                label = json.loads(labels[i].strip())
            else:
...
class CodahProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, data_file):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
        return self._create_examples(self._read_csv(os.path.join(data_dir, data_file)), "train")
...
class SiqaProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, labels, type, threshold=0.5):
        """Creates examples for the training and dev sets."""
        ...
        for line, label in tqdm.tqdm(zip(lines, labels), desc="read arc data"):
            ...
            # filter based on QAP/KL
            if type == "train":
                if 'qap' in data_raw:
                    if data_raw['qap'] < threshold:
                        break
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        if ex_index < 2:
            ...
            for choice_idx, (input_ids, attention_mask, token_type_ids) in enumerate(choices_features):
                ...
                logger.info("label: {}".format(json.dumps(label)))
