...
def roberta_train_and_eval(arg_string):
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        fresult = open('cnet_filter_param_results.jsonl', 'a+')
...
def optimize_roberta(use_task_model=False):
    ...
    def parse_logits_file(data_file, logits_file):
        ...
        assert len(samples) == len(logits)
        return samples, np.array([np.array(l) for l in logits])

    def select_by_qap(data_file, logits_file, lower, higher=1.0):
        """True distribution is taken from data_file1"""

        # read samples and logits file
        samples, logits = parse_logits_file(data_file, logits_file)
    
        label2idx = {"A": 1, "B": 2, "C": 3, "D": 4, "E": 5}

        probs = softmax(logits)
        probs_qap = []
        print("Converting logits")
        for i, sample in enumerate(samples):
            label_idx = label2idx[sample['answerKey']] - 1
        ...
        ranked_by_qap_idxs = sorted(idxs, key=lambda i: probs_qap[i], reverse=True)
        for n, idx in enumerate(ranked_by_qap_idxs):
            if probs_qap[idx] <= lower:
                continue
            if probs_qap[idx] >= higher:
                continue
            selected_idxs.append(idx)
        random.shuffle(selected_idxs)
        
        counter = 0
        with open(data_file.replace('.jsonl', '-filtered-%s.jsonl' % lower), 'w') as f:
            for n, idx in tqdm(enumerate(selected_idxs)):
                counter += 1
                f.write(json.dumps(samples[idx]) + '\n')
        print("%s samples written to file" % counter)
        
    def roberta_wrapper(lower):
        ...
        select_by_qap('../data/cqa/cnet-train-gpt-large-co-100k.jsonl', '../data/cqa/cnet-train-gpt-large-co-100k-logits.txt', lower)
        
        args_list = ['--model_type', 'roberta',
                     '--task_name', 'cqa',
                     '--model_name_or_path', 'roberta-large',
                     '--train_file', 'cnet-train-gpt-large-co-100k-filtered-%s.jsonl' % lower,
                     '--eval_file', 'dev_rand_split.jsonl',
                     '--data_dir', '../data/cqa',
                     '--output_dir', './models/cnet-roberta-large/filter-bayes-' + str(lower),
                     '--logging_steps', '50',
                     '--do_train', '--do_eval',
                     '--num_train_epochs', '1',
                     '--max_seq_length', '70',
